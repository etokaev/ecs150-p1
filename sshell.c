...
struct LinkedList
{
	char* arrData[MAX_NUM_ARGS];//array of pointers that stores parsed args of a cmd and cmd itself
	struct LinkedList *next;
	int pid;
	int status;//return status of the cmds when piping
};
...
int cmprStr(node headNode, char *str1)
{
	...
	return result;
}//compares the string and if equals, returns 0
...
int display_prompt()
{
	printf("sshell$ ");
	return 0;
}//redundant after using the provided prompt display (for compatibility with the testing script
...
}//prints each element of the array within the given node (not every node)
...
void builtinCommands(node headNode)
{
	...
	isPwd(headNode);
	isCD(headNode);
}//checks whether pwd or cd or exit cmd has been used
...
}//parses user input into cmds and adds them to the linkedList in the final format, ready for pipe creation in the makePipe
...
int countPipes(char* lineInput)
{
	...
	return counter;
}//counts number of | chars in the user input

void resetGlobalVars()
{
	isError = 0;
	isInterrupt = 0;
}//resets global interrupt variables
...
}//checks whether user input starts inappropriately
